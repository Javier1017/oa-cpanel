...
import type { LeaveBalanceItem, Params } from './data.d';

// mock - generate withdrawal list
const genList = (current: number, pageSize: number) => {
  const tableListDataSource: LeaveBalanceItem[] = [];
  const departmentArray = ['Admin', 'Sales', 'IT', 'HR', 'Marketing'];
  const leaveGroupArray = ['Standard', 'MY', 'PHP', 'Lorem ipsum'];

  for (let i = 0; i < pageSize; i += 1) {
    const randomDepartment = Math.floor(Math.random() * departmentArray.length);
    const randomLeaveGroup = Math.floor(Math.random() * leaveGroupArray.length);
    tableListDataSource.push({
      ...
      leaveGroup: leaveGroupArray[randomLeaveGroup],
    });
  }
...
function getLeaveBalances(req: Request, res: Response, u: string) {
  let realUrl = u;
...
export default {
  'GET /api/leave-balances': getLeaveBalances,
};
