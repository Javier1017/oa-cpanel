...
import type { AttendanceItem, Params } from './data.d';

// mock - generate withdrawal list
const genList = (current: number, pageSize: number) => {
  const tableListDataSource: AttendanceItem[] = [];
  ...
  for (let i = 0; i < pageSize; i += 1) {
    ...
    tableListDataSource.push({
      ...
      duration: `${randomHour}H ${randomMinutes}m`,
      createdAt: Date.now() - Math.floor(Math.random() * 999999),
...
function getAttendance(req: Request, res: Response, u: string) {
  let realUrl = u;
